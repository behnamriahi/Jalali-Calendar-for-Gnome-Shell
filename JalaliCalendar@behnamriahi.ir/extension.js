...
const PersianCalendar = new Lang.Class({
    ...
    _init: function () {
        ...
        nowroozIcon.connect('clicked', function () {
            ...
            let now = new Date();
            let pdate = JDate.JDate.gregorianToJalali(
                now.getFullYear(),
                now.getMonth() + 1,
                now.getDate()
            );
        ...
        this.menu.connect('open-state-changed', Lang.bind(that, function (menu, isOpen) {
            if (isOpen) {
                let now = new Date();
                now = JDate.JDate.gregorianToJalali(now.getFullYear(), now.getMonth() + 1, now.getDate());
    ...
    _updateDate: function (skip_notification, force) {
        ...
        // convert to Persian
        _date = JDate.JDate.gregorianToJalali(_date.getFullYear(), _date.getMonth() + 1, _date.getDate());
        ...
        _date = str.format(_date.day + ' ' + JDate.JDate.p_month_names[_date.month - 1] + ' ' + _date.year);
    ...
    _generateConverterPart: function () {
        ...
        let fromJalali = new St.Button({
            reactive: true,
            can_focus: true,
            track_hover: true,
            x_expand: true,
            label: _('از قمری'),
            accessible_name: 'fromJalali',
            style_class: 'popup-menu-item button pcalendar-button fromJalali'
        });
        fromJalali.connect('clicked', Lang.bind(this, this._toggleConverter));
        fromJalali.TypeID = ConverterTypes.fromJalali;

        middleBox.add(fromJalali);
    ...
    _onModifyConverter: function () {
        ...
        case ConverterTypes.fromGregorian:
            pDate = JDate.JDate.gregorianToJalali(year, month, day);
            hDate = JalaliDate.JalaliDate.toJalali(year, month, day);
            break;

        case ConverterTypes.fromPersian:
            gDate = JDate.JDate.jalaliToGregorian(year, month, day);
            hDate = JalaliDate.JalaliDate.toJalali(gDate.year, gDate.month, gDate.day);
            break;

        case ConverterTypes.fromJalali:
            gDate = JalaliDate.JalaliDate.fromJalali(year, month, day);
            pDate = JDate.JDate.gregorianToJalali(gDate.year, gDate.month, gDate.day);
            break;
        ...
        if (hDate) {
            let button = new St.Button({
                label: str.format(
                    this._calendar.format(
                        Schema.get_string('jalali-display-format'),
                        hDate.day,
                        hDate.month,
                        hDate.year,
                        $dayOfWeek,
