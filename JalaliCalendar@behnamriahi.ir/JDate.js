...
JDate.jalaliToGregorian = function (py, pm, pd) {
    py = parseInt(py) - 979;
    pm = parseInt(pm) - 1;
    pd = parseInt(pd) - 1;
    ...
    for (let i = 0; i < pm; i++) {
        p_day_no += JDate.p_days_in_month[i];
    }
    ...
    let i = 0;
    for (; g_day_no >= JDate.g_days_in_month[i] + (i === 1 && leap); i++) {
        g_day_no -= JDate.g_days_in_month[i] + (i === 1 && leap);
    }

    return {year: gy, month: i + 1, day: g_day_no + 1};
};

JDate.checkDate = function (py, pm, pd) {
    return !(py < 0 || py > 32767 || pm < 1 || pm > 12 || pd < 1 || pd >
    (JDate.p_days_in_month[pm - 1] + (pm === 12 && !((py - 979) % 33 % 4))));
};

JDate.gregorianToJalali = function (gy, gm, gd) {
    gy = parseInt(gy) - 1600;
    gm = parseInt(gm) - 1;
    gd = parseInt(gd) - 1;
    ...
    for (let i = 0; i < gm; ++i) {
        g_day_no += JDate.g_days_in_month[i];
    }
    /* leap and after Feb */
    ...
    let i = 0;
    for (; i < 11 && p_day_no >= JDate.p_days_in_month[i]; ++i) {
        p_day_no -= JDate.p_days_in_month[i];
    }
