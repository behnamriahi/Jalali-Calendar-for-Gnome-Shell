...
persian.prototype = {
    ...
    addSpecificEvents: function (pdate) {
        let date = new Date();
        date = pdate.gregorianToJalali(date.getFullYear(), date.getMonth() + 1, date.getDate());
        let year = date.year;
        ...
        for (let i = 1; i <= 7; i++) {
            let p_ts = pdate.jalaliToGregorian(year, 1, i);
            p_ts = new Date(p_ts.year, p_ts.month - 1, p_ts.day, 5);
            /* do not remove this 5 :D */
            if (p_ts.getDay() === 3) {
                let dummy_date = pdate.gregorianToJalali(p_ts.getFullYear(), p_ts.getMonth() + 1, p_ts.getDate());
        ...
        for (let i = 0; i < 6; i++) {
            let p_ts = pdate.jalaliToGregorian(year, 12, 29 + leap - i);
